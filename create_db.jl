...
begin
	...
	local_sst = LocalResource(sst, :autoplay => "", :loop => "", :width => 1500, :height => 800)
end
...
begin
	...
	PlutoUI.ExperimentalLayout.vbox(
		[
			...
			A key component of ocean circulation is *mixing* which can increase both the lateral and vertical transportat of heat and salt.
			Lateral mixing refers to mixing that occurs along surfaces of constant density whereas vertical mixing is across these constant density surfaces.
...
begin
	...
	local_dc = LocalResource(dc, :autoplay => "", :loop => "", :width => 1100, :height => 500)
end

# ╔═╡ bdcfcd1b-6973-4c41-b1d5-08a2ec877e46
begin
	convection_cell = PlutoRunner.currently_running_cell_id[] |> string
	local_convection = LocalResource(convection, :autoplay => "", :loop => "", :width => 1000, :height => 800)
end

# ╔═╡ 46039523-887d-4df6-9be7-170c929fcf72
begin
	convection_tracers_cell = PlutoRunner.currently_running_cell_id[] |> string
	local_convection_tracers = LocalResource(convection_tracers, :autoplay => "", :loop => "", :width => 1000, :height => 800)
end

# ╔═╡ 981b3dc4-f4d7-4466-a0ff-18d1d4938c24
begin
	cabbeling_cell = PlutoRunner.currently_running_cell_id[] |> string
	local_cabbeling = LocalResource(cabbeling, :autoplay => "", :loop => "", :width => 1000, :height => 800)
end
	...
	PlutoUI.ExperimentalLayout.vbox([
			...
			PlutoUI.ExperimentalLayout.hbox([
					...
					""",
							PlutoUI.ExperimentalLayout.hbox([
					md""" 
				In the ocean, the vertical transport of heat and other biogeochemical tracers is **relatively slow** compared to the lateral transport [1].
				...
				local_convection], style=Dict("justify-content" => "center", "align-items" => "center", "gap" => "1em")),
					...
					Looking at the salinity and temperature, which determine the density, we can see that the convection is very active until these tracers are homogeneous over the domain.

					All animations on this dashboard are from simulations built with `Oceananigans.jl` [2].
					"""], 
						style=Dict("justify-content" => "center", "align-items" => "top", "gap" => "1em")),
										md"""

[1] Waterhouse, A. F., and Coauthors, 2014: Global patterns of diapycnal mixing from
measurements of the turbulent dissipation rate. Journal of Physical Oceanography, 44, 1854–1872, https://doi.org/10.1175/JPO-D-13-0104.1.\
[2] Ramadhan, A., and Coauthors, 2020: Oceananigans.jl: Fast and friendly geophysical fluid dynamics on gpus. Journal of Open Source Software, 5, 2018, https://doi.org/10.21105/joss.02018.
				""",
					], style=Dict("justify-content" => "center", "align-items" => "center", "gap" => "1em")),
					PlutoUI.ExperimentalLayout.vbox([
					html"""
				...
				""",
						PlutoUI.ExperimentalLayout.hbox([
				md"""
				When temperature is weakening the stability of a water column (i.e. when cold water sits atop warm water), and there are no external sources of mixing or turbulence, *diffusive convection* occurs.
				...
				When warm/salty water ovelies cold/fresh water, a different convective instability called *salt fingering* occurs.
				""",
				local_dc], style=Dict("justify-content" => "center", "align-items" => "top", "gap" => "0em")),
				html"""
				...
				For more information on how non-linear processes influence vertical mixing see the research poster *Cabbeling's influence on mixing in Direct Numerical Simulations*.
				"""], style=Dict("justify-content" => "center", "align-items" => "top", "gap" => "1em"))
					],style=Dict("justify-content" => "center", "align-items" => "center", "gap" => "1em"))
			],
			style=Dict("justify-content" => "center", "align-items" => "center", "gap" => "2.5em"))
		],
		style=Dict("justify-content" => "center", "background" => "border-radius" => "50px", "padding" => "20px", "margin" => "30px", "font-size" => "45px"))
end
